...
import { loadUkiyoeData, getLocalizedField } from '../data/ukiyoe'
...
function FixedPeriodPanel({ period, index, language, isArtworkDetailOpen, showGallery }) {
  ...
  return (
    <motion.div
      className="timeline-fixed-panel"
...
export default function TimelinePage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setPeriods(data)
        setScrollProgresses(data.map(() => 0))
      } catch (err) {
  ...
  // ネイティブスクロール: スクロール位置から currentSection と scrollProgresses を更新
  useEffect(() => {
      ...
      <div className="timeline-native-scroll">
        {periods.map((period, index) => (
          <TimelineDetailSection
            ...
            scrollProgress={scrollProgresses[index] ?? 0}
            useNativeScroll
